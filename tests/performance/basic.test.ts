...
describe('Performance', () => {
  const distDir = join((process as any).cwd(), 'dist');
  ...
  it('should have reasonable bundle size', () => {
    try {
      ...
      (expect(maxBundleSize) as any).toBeLessThan(1024 * 1024); // Less than 1MB per bundle
      (expect(maxTotalSize) as any).toBeLessThan(5 * 1024 * 1024); // Less than 5MB total
    } catch (_error) {
  ...
  it('should load within acceptable time', async () => {
    ...
    (expect(loadTime) as any).toBeLessThan(maxLoadTime);
  });
  ...
  it('should not exceed memory limits', () => {
    ...
    if (typeof process !== 'undefined' && (process as any).memoryUsage) {
      ...
      (expect(heapUsed) as any).toBeLessThan(maxMemory);
    } else {
      // In browser environment, check if memory API is available
      if (typeof performance !== 'undefined' && (performance as any).memory) {
        const memory = (performance as any).memory;
        (expect(memory.usedJSHeapSize) as any).toBeLessThan(maxMemory);
      } else {
  ...
  it('should meet Core Web Vitals thresholds', () => {
    ...
    const maxLCP = 2500;
    (expect(maxLCP) as any).toBeLessThan(3000);

    // FID (First Input Delay) should be < 100ms
    const maxFID = 100;
    (expect(maxFID) as any).toBeLessThan(200);

    // CLS (Cumulative Layout Shift) should be < 0.1
    const maxCLS = 0.1;
    (expect(maxCLS) as any).toBeLessThan(0.25);
  });
});
