...
test.describe('400% Zoom Testing', () => {
  test.describe('Page Layout at 400% Zoom', () => {
    for (const testPage of TEST_PAGES) {
      test(`${testPage.name} - No horizontal scrolling`, async ({ page }: { page: Page }) => {
        // Set viewport to simulate 400% zoom
        // At 400% zoom, a 1920px wide screen becomes effectively 480px
        await page.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
        
        // Navigate to page
        const baseURL = page.context().baseURL || 'http://localhost:3000';
        const appUrl = getBaseUrl(testPage.app, baseURL);
        await page.goto(`${appUrl}${testPage.url}`);
      ...
      test(`${testPage.name} - Content reflows properly`, async ({ page }: { page: Page }) => {
        // Set viewport to simulate 400% zoom
        await page.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
      ...
      test(`${testPage.name} - Interactive elements accessible`, async ({ page }: { page: Page }) => {
        // Set viewport to simulate 400% zoom
        await page.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
      ...
      test(`${testPage.name} - Text remains readable`, async ({ page }: { page: Page }) => {
        // Set viewport to simulate 400% zoom
        await page.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
