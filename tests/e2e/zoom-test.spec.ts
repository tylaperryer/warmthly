...
test.describe('400% Zoom Testing', () => {
  test.describe('Page Layout at 400% Zoom', () => {
    for (const page of TEST_PAGES) {
      test(`${page.name} - No horizontal scrolling`, async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        ...
        expect(horizontalScroll.scrollWidth).toBeLessThanOrEqual(horizontalScroll.clientWidth + MAX_HORIZONTAL_SCROLL);
      });
      
      test(`${page.name} - Content reflows properly`, async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        // Set viewport to simulate 400% zoom
        await browserPage.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
      ...
      test(`${page.name} - Interactive elements accessible`, async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        // Set viewport to simulate 400% zoom
        await browserPage.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
      ...
      test(`${page.name} - Text remains readable`, async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        // Set viewport to simulate 400% zoom
        await browserPage.setViewportSize({ width: 1920 / ZOOM_LEVEL, height: 800 });
