...
test.describe('Screen Reader Accessibility Testing', () => {
  for (const page of TEST_PAGES) {
    test.describe(`${page.name}`, () => {
      test('should have proper heading hierarchy', async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
      ...
      test('should have all form labels properly associated', async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        const appUrl = getBaseUrl(page.app, baseURL || 'http://localhost:3000');
        await browserPage.goto(`${appUrl}${page.url}`);
        await browserPage.waitForLoadState('networkidle');
      ...
      test('should have all buttons with accessible names', async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        const appUrl = getBaseUrl(page.app, baseURL || 'http://localhost:3000');
        await browserPage.goto(`${appUrl}${page.url}`);
        await browserPage.waitForLoadState('networkidle');
      ...
      test('should have all links with descriptive text', async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        const appUrl = getBaseUrl(page.app, baseURL || 'http://localhost:3000');
        await browserPage.goto(`${appUrl}${page.url}`);
        await browserPage.waitForLoadState('networkidle');
      ...
      test('should have skip link that works', async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        const appUrl = getBaseUrl(page.app, baseURL || 'http://localhost:3000');
        await browserPage.goto(`${appUrl}${page.url}`);
        await browserPage.waitForLoadState('networkidle');
      ...
      test('should have proper ARIA attributes', async ({ page: browserPage, baseURL }: { page: Page; baseURL: string | undefined }) => {
        const appUrl = getBaseUrl(page.app, baseURL || 'http://localhost:3000');
        await browserPage.goto(`${appUrl}${page.url}`);
        await browserPage.waitForLoadState('networkidle');
        ...
        const ariaCheck = await browserPage.evaluate(() => {
          ...
          const hiddenInteractive = document.querySelectorAll('[aria-hidden="true"][role="button"], [aria-hidden="true"][role="link"], [aria-hidden="true"]button, [aria-hidden="true"]a');
          if (hiddenInteractive.length > 0) {
            issues.push(`Found ${hiddenInteractive.length} interactive elements with aria-hidden="true"`);
          }
