...
test.describe('Report Submission', () => {
  ...
  test('should find report submission form', async ({ page }: { page: Page }) => {
    ...
    const reportLink = page.locator('a:has-text("Report"), a:has-text("Contact"), a:has-text("Concern")').first();
  ...
  test('should validate required fields', async ({ page }: { page: Page }) => {
    ...
    const reportLink = page.locator('a:has-text("Report"), a:has-text("Contact")').first();
    if (await reportLink.count() > 0) {
      await reportLink.click();
      await page.waitForTimeout(1000);
