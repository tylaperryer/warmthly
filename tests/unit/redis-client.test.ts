...
describe('Redis Client', () => {
  ...
  it('should handle connection errors', async () => {
    ...
    const { createClient } = await import('redis');
    const redisUrl = typeof process !== 'undefined' && process.env ? (process.env as any).REDIS_URL : 'redis://invalid:6379';
  ...
  it('should set up event handlers', async () => {
    ...
    const { createClient } = await import('redis');
    const redisUrl = typeof process !== 'undefined' && process.env ? (process.env as any).REDIS_URL : 'redis://localhost:6379';
    const mockClient = createClient({ url: redisUrl });
