...
describe('Login Handler', () => {
  ...
  it('should return 405 for non-POST requests', async () => {
    const { login } = await import('@api/auth/index.js');
    const req = {
      method: 'GET',
      body: {},
      url: '',
  ...
  it('should return 500 if JWT_SECRET is not configured', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      error: { message: 'Authentication system not configured.' },
    });
  });
  ...
  it('should return JWT token for correct password', async () => {
    ...
    expect(jwt.default.sign).toHaveBeenCalledWith({ user: 'admin' }, 'jwt-secret', {
      expiresIn: '8h',
    });
