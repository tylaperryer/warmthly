 ...
 */
async function getEmailsHandler(req: Request, res: Response): Promise<Response> {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    try {
      ...
      if (kvError instanceof Error) {
        logger.error('[get-emails] Error fetching from Redis:', {
          ...
          stack: kvError.stack,
          name: kvError.name,
          code: (kvError as { code?: string }).code,
        });
    ...
    // Parse emails
    const parsedEmails: EmailData[] = emails
      .map((email: string, index: number) => {
        try {
          return JSON.parse(email) as EmailData;
        } catch (e: unknown) {
      ...
      .filter((email: EmailData | null): email is EmailData => email !== null);
