...
async function checkRateLimit(
  ...
  try {
    ...
    if (failureMode === RateLimitFailureMode.FAIL_CLOSED) {
      ...
      const memoryStore = (globalThis as { rateLimitStore?: Map<string, { count: number; resetTime: number }> }).rateLimitStore ?? new Map();
      
      if (!(globalThis as { rateLimitStore?: Map<string, { count: number; resetTime: number }> }).rateLimitStore) {
        (globalThis as { rateLimitStore?: Map<string, { count: number; resetTime: number }> }).rateLimitStore = memoryStore;
      }
