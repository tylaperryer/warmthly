 ...
 */
export function withCSRFProtection<T extends { readonly method: string; readonly body?: { readonly csrfToken?: string; [key: string]: unknown } }>(
  handler: (req: T, res: { status: (code: number) => { json: (data: unknown) => unknown } }) => Promise<unknown> | unknown,
  getSessionToken: (req: T) => string | null | undefined
) {
  return async (req: T, res: { status: (code: number) => { json: (data: unknown) => unknown } }): Promise<unknown> => {
