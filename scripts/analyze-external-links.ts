 ...
 */
function extractExternalLinks(content: string, basePath: string): Array<{
  url: string;
  line: number;
  anchorText: string;
  attributes: string;
...
function analyzeExternalLinks(): void {
  ...
  console.log(`   Links with nofollow: ${withNofollow} (${((withNofollow / allLinks.length) * 100).toFixed(1)}%)`);
  console.log(`   Links with noopener/noreferrer: ${withNoopener} (${((withNoopener / allLinks.length) * 100).toFixed(1)}%)`);
