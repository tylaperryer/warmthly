...
function getUniqueCharacters(text: string): string {
  ...
  for (const char of text) {
    if (char.charCodeAt(0) > 32) { // Skip control characters
      chars.add(char);
    }
  }
...
function subsetFontPyftsubset(
  ...
  try {
    ...
    const ranges = unicodeRanges.split(',').map(r => r.trim()).join(',');
    
    const command = `pyftsubset "${sourceFullPath}" --unicodes="${ranges}" --output-file="${outputFullPath}" --flavor=woff2 --with-zopfli`;
 ...
 */
function subsetFontGlyphhanger(
  sourcePath: string,
  outputPath: string,
  text: string
): void {
  ...
  try {
    ...
    const command = `glyphhanger --subset="${sourceFullPath}" --formats=woff2 --output="${outputDir}" --text="${escapedText}"`;
...
function main(): void {
  ...
  console.log('Example: src: url("/fonts/Inter-VariableFont_opsz,wght-subset.woff2") format("woff2");');
}
