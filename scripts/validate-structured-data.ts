...
function extractStructuredData(html: string): Array<{ json: object; line: number }> {
  ...
  lines.forEach((line, index) => {
    ...
    const scriptMatch = line.match(/<script[^>]*type=["']application\/ld\+json["'][^>]*>(.*?)<\/script>/s);
    if (scriptMatch) {
      try {
...
function validateStructuredData(): void {
  ...
  // Report results
  console.log(`Found ${totalSchemas} structured data schema(s) across ${htmlFiles.length} file(s)\n`);
