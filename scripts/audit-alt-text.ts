      ...
      if (!hasAlt) {
        ...
        if (!hasAriaHidden && !hasRole) {
          issues.push({
            file: filePath.replace(projectRoot, ''),
            line: index + 1,
            issue: 'Empty alt text - add alt="" for decorative images or descriptive alt for meaningful images',
            element: imgTag.substring(0, 100),
          });
        }
      ...
      if (!hasAriaLabel && !hasTitle && !hasAriaHidden && !hasRole) {
        ...
        if (!/decorative|decoration|spacer/i.test(context)) {
          issues.push({
            file: filePath.replace(projectRoot, ''),
            line: index + 1,
            issue: 'SVG missing aria-label or title - add aria-label or aria-hidden="true" for decorative SVGs',
