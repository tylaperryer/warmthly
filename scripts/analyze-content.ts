...
function analyzeContent(): void {
  ...
  htmlFiles.forEach(file => {
    ...
    if (words.length > 3000) {
      issues.push(
        `Content very long (${words.length} words) - consider breaking into multiple pages`
      );
    }
  ...
  sortedByLength.slice(0, 10).forEach((analysis, idx) => {
    ...
    console.log(
      `      Words: ${analysis.wordCount} | Paragraphs: ${analysis.paragraphCount} | Headings: ${analysis.headingCount}`
    );
  ...
  report += `- Average word count: ${Math.round(
