 ...
 */
function extractMetaTag(html: string, type: 'title' | 'description' | 'og:title' | 'og:description' | 'og:image' | 'canonical'): string | null {
  switch (type) {
    case 'title':
    ...
    case 'og:title':
      ...
      return ogTitleMatch ? ogTitleMatch[1].trim() : null;
    
    case 'og:description':
      const ogDescMatch = html.match(/<meta[^>]*property=["']og:description["'][^>]*content=["']([^"']+)["']/i);
      return ogDescMatch ? ogDescMatch[1].trim() : null;
    
    case 'og:image':
      const ogImageMatch = html.match(/<meta[^>]*property=["']og:image["'][^>]*content=["']([^"']+)["']/i);
      return ogImageMatch ? ogImageMatch[1].trim() : null;
    ...
    if (!description) {
      ...
      if (description.length < 120) {
        ...
        issues.push({
          ...
          recommendation: 'Shorten description to 120-160 characters (will be truncated in search results)',
          severity: 'warning',
        });
      }
      ...
      if (!hasCTA) {
        issues.push({
          ...
          recommendation: 'Include action words (learn, discover, explore) to improve click-through',
          severity: 'info',
        });
      }
    }
