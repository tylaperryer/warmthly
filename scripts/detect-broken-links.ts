 ...
 */
function extractLinks(content: string, basePath: string): Array<{ url: string; line: number; context: string }> {
 ...
 */
async function checkExternalUrl(url: string): Promise<{ valid: boolean; status?: number; error?: string }> {
  try {
...
async function detectBrokenLinks(): Promise<void> {
  ...
  console.log(`Checking ${Math.min(externalLinks.length, 20)} external links (limited to 20 for performance)...\n`);
