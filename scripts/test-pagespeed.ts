 ...
 */
async function testPageSpeed(url: string, strategy: 'mobile' | 'desktop'): Promise<PageSpeedResult | null> {
  const apiKey = process.env.PAGESPEED_API_KEY;
  
  if (!apiKey) {
    console.warn(`⚠️  PAGESPEED_API_KEY not set. Skipping PageSpeed Insights test for ${url}`);
    console.warn('   Get a free API key at: https://developers.google.com/speed/docs/insights/v5/get-started');
    return null;
  }
...
async function testAllPages(): Promise<void> {
  ...
  // Summary
  const avgMobileScore = results
    .filter(r => r.mobile)
    .reduce((sum, r) => sum + (r.mobile?.score || 0), 0) / results.filter(r => r.mobile).length;
  
  const avgDesktopScore = results
    .filter(r => r.desktop)
    .reduce((sum, r) => sum + (r.desktop?.score || 0), 0) / results.filter(r => r.desktop).length;
