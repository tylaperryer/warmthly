  ...
  lines.forEach((line, index) => {
    ...
    while ((match = buttonRegex.exec(line)) !== null) {
      ...
      if (!hasAriaLabel && !hasAriaLabelledBy && !hasTitle) {
        ...
        const isIconOnly =
          /<button[^>]*>[\s\S]*?<[^>]+>[\s\S]*?<\/button>/i.test(line) &&
    ...
    while ((match = inputRegex.exec(line)) !== null) {
      ...
      if (
        ...
        // Placeholder is not sufficient for accessibility - always report missing label
        issues.push({
          file: relativePath,
          line: index + 1,
          type: 'error',
          ...
          recommendation:
            'Add <label> element with for attribute matching input id, or use aria-label',
        });
      }
    }
      ...
      if (imgMatch) {
        ...
        if (!hasAlt) {
          issues.push({
            ...
            recommendation:
              'Add alt="" for decorative images or descriptive alt for meaningful images',
          });
        }
      }
    }
    ...
    if (index === 0 && headings.length > 0) {
      ...
      for (let i = 0; i < headings.length - 1; i++) {
        ...
        const nextLevel = headings[i + 1];
        if (currentLevel !== undefined && nextLevel !== undefined && nextLevel - currentLevel > 1) {
          issues.push({
            file: relativePath,
            line: 1,
            type: 'warning',
            issue: `Skipped heading level: h${currentLevel} to h${nextLevel}`,
