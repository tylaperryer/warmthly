  ...
  const urlEntries = urls
    .map((url) => {
      ...
      const imageEntries = url.images?.map((img) => {
        ...
        const geoLocation = img.geoLocation ? `      <image:geo_location>${escapeXml(img.geoLocation)}</image:geo_location>\n` : '';
        const license = img.license ? `      <image:license>${escapeXml(img.license)}</image:license>\n` : '';
        
        return `    <image:image>\n      <image:loc>${escapeXml(img.loc)}</image:loc>\n${title}${caption}${geoLocation}${license}    </image:image>`;
      }).join('\n') || '';
      
      // Generate video entries
      const videoEntries = url.videos?.map((video) => {
        const contentLoc = video.contentLoc ? `      <video:content_loc>${escapeXml(video.contentLoc)}</video:content_loc>\n` : '';
        const playerLoc = video.playerLoc ? `      <video:player_loc>${escapeXml(video.playerLoc)}</video:player_loc>\n` : '';
        const duration = video.duration ? `      <video:duration>${video.duration}</video:duration>\n` : '';
        const expirationDate = video.expirationDate ? `      <video:expiration_date>${video.expirationDate}</video:expirationDate>\n` : '';
        const rating = video.rating ? `      <video:rating>${video.rating}</video:rating>\n` : '';
        const viewCount = video.viewCount ? `      <video:view_count>${video.viewCount}</video:view_count>\n` : '';
        const publicationDate = video.publicationDate ? `      <video:publication_date>${video.publicationDate}</video:publicationDate>\n` : '';
        const familyFriendly = video.familyFriendly !== undefined ? `      <video:family_friendly>${video.familyFriendly ? 'yes' : 'no'}</video:family_friendly>\n` : '';
        const requiresSubscription = video.requiresSubscription !== undefined ? `      <video:requires_subscription>${video.requiresSubscription ? 'yes' : 'no'}</video:requires_subscription>\n` : '';
        const live = video.live !== undefined ? `      <video:live>${video.live ? 'yes' : 'no'}</video:live>\n` : '';
        const tags = video.tag?.map(tag => `      <video:tag>${escapeXml(tag)}</video:tag>\n`).join('') || '';
        const category = video.category ? `      <video:category>${escapeXml(video.category)}</video:category>\n` : '';
