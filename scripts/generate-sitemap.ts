...
try {
  const configModule = await import('../lego/config/warmthly-config.js');
  WARMTHLY_CONFIG = configModule.WARMTHLY_CONFIG;
  if (!WARMTHLY_CONFIG) {
    throw new Error('WARMTHLY_CONFIG is not exported from warmthly-config.js');
  }
} catch (error) {
  console.error('❌ Error loading WARMTHLY_CONFIG:', error);
  console.error('   Make sure warmthly-config.js exists and exports WARMTHLY_CONFIG');
  process.exit(1);
}
...
function generatePostSitemap(): string {
  ...
  const configRoutes = WARMTHLY_CONFIG.routes;
  if (routes && configRoutes) {
    if (routes.report) {
      urls.push({
