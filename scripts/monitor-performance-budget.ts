...
function loadPerformanceData(): Partial<PerformanceBudget> {
  ...
  if (process.env.FID) {
    performanceData.fid = parseFloat(process.env.FID);
  }
  if (process.env.CLS) {
    performanceData.cls = parseFloat(process.env.CLS);
  }
  if (process.env.FCP) {
    performanceData.fcp = parseFloat(process.env.FCP);
  }
  if (process.env.TTFB) {
    performanceData.ttfb = parseFloat(process.env.TTFB);
  }
  if (process.env.TBT) {
    performanceData.totalBlockingTime = parseFloat(process.env.TBT);
  }
  if (process.env.SPEED_INDEX) {
    performanceData.speedIndex = parseFloat(process.env.SPEED_INDEX);
  }
