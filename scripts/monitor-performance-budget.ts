 ...
 */
function checkBudget(metrics: Partial<PerformanceBudget>, budgets: PerformanceBudget): BudgetViolation[] {
...
function generateReport(violations: BudgetViolation[], budgets: PerformanceBudget): string {
  ...
  report += `|--------|--------|--------|\n`;
  report += `| LCP (Largest Contentful Paint) | < ${budgets.lcp}ms | ${violations.find(v => v.metric === 'LCP') ? '❌' : '✅'} |\n`;
  report += `| FID (First Input Delay) | < ${budgets.fid}ms | ${violations.find(v => v.metric === 'FID') ? '❌' : '✅'} |\n`;
  report += `| CLS (Cumulative Layout Shift) | < ${budgets.cls} | ${violations.find(v => v.metric === 'CLS') ? '❌' : '✅'} |\n`;
  report += `| FCP (First Contentful Paint) | < ${budgets.fcp}ms | ${violations.find(v => v.metric === 'FCP') ? '❌' : '✅'} |\n`;
  report += `| TTFB (Time to First Byte) | < ${budgets.ttfb}ms | ${violations.find(v => v.metric === 'TTFB') ? '❌' : '✅'} |\n`;
  report += `| TBT (Total Blocking Time) | < ${budgets.totalBlockingTime}ms | ${violations.find(v => v.metric === 'TBT') ? '❌' : '✅'} |\n`;
  report += `| Speed Index | < ${budgets.speedIndex}ms | ${violations.find(v => v.metric === 'SPEEDINDEX') ? '❌' : '✅'} |\n\n`;
  ...
  try {
    ...
    if (violations.length === 0) {
      ...
      violations.forEach((violation) => {
        ...
        console.log(`   ${icon} ${violation.metric}: ${violation.current} (budget: ${violation.budget})`);
      });
