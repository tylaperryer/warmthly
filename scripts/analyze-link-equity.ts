...
function getUrlFromPath(filePath: string): string {
  ...
  if (relative.includes('/apps/admin/')) {
    ...
    return `https://admin.warmthly.org${path.replace('/index.html', '/').replace('.html', '.html')}`;
  }
...
function analyzeLinkEquity(): void {
  ...
  const homepage = Array.from(pageInfo.values()).find(p => p.url.includes('warmthly.org/') && !p.url.includes('/'));
  if (homepage) {
  ...
  if (highPriorityLowLinks.length > 0) {
    console.log(`   - Increase internal links to ${highPriorityLowLinks.length} high-priority page(s)`);
  }
