 ...
 */
const CSS_FILES = [
  join(projectRoot, 'lego/styles/variables.css'),
  join(projectRoot, 'lego/styles/base.css'),
  join(projectRoot, 'lego/styles/components.css'),
];

/**
 * Output file for critical CSS
 */
const OUTPUT_FILE = join(projectRoot, 'lego/styles/critical.css');
...
async function extractCriticalCSS(cssContent: string): Promise<string> {
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes('{') && !trimmed.includes('}')) {
      // Start of a rule
      inRule = true;
      const selectorPart = trimmed.split('{')[0];
      currentSelector = selectorPart ? selectorPart.trim() : '';
      ruleContent = [trimmed];
