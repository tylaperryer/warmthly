...
function extractKeywords(content: string, title: string): string[] {
  ...
  const commonWords = ['this', 'that', 'with', 'from', 'have', 'will', 'your', 'their', 'there', 'these', 'those'];
...
function calculateRelevance(
  ...
  const titleMatches = sourceKeywords.filter(k => titleWords.some(tw => tw.includes(k) || k.includes(tw))).length;
...
function generateSuggestions(): void {
    ...
    lines.forEach((line, index) => {
      ...
      if (/<p[^>]*>[\s\S]{50,}/i.test(line) || /<li[^>]*>[\s\S]{30,}/i.test(line)) {
        ...
        candidates.forEach((candidate) => {
          ...
          const reason = commonKeywords.length > 0
  ...
  filteredSuggestions.slice(0, 10).forEach((suggestion, idx) => {
    ...
    console.log(`      → ${suggestion.suggestedLink.text} (${(suggestion.suggestedLink.relevance * 100).toFixed(1)}% relevance)`);
  });
  console.log('');
}
