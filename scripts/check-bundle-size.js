...
const BUDGETS = {
  // JavaScript budgets
  js: {
    maxInitial: 200, // Max initial JS bundle (200KB)
    maxTotal: 500,   // Max total JS across all chunks (500KB)
    maxChunk: 150,   // Max individual chunk size (150KB)
  },
  // CSS budgets
  css: {
...
function checkBundleSizes(buildDir = 'dist') {
  ...
  stats.js.chunks.forEach((chunk) => {
    ...
    if (chunk.size > BUDGETS.js.maxChunk * 1024) {
      ...
      console.log(`  ${colors.yellow}⚠ Chunk exceeds budget: ${chunk.path} (${chunkKB} KB)${colors.reset}`);
    }
  });
  ...
  stats.css.files.forEach((file) => {
    ...
    if (file.size > BUDGETS.css.maxFile * 1024) {
      ...
      console.log(`  ${colors.yellow}⚠ File exceeds budget: ${file.path} (${fileKB} KB)${colors.reset}`);
    }
  });
  ...
  stats.fonts.files.forEach((file) => {
    ...
    if (file.size > BUDGETS.fonts.maxFile * 1024) {
      ...
      console.log(`  ${colors.yellow}⚠ Font exceeds budget: ${file.path} (${fileKB} KB)${colors.reset}`);
    }
  });
