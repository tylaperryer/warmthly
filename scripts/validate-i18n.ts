...
function validateI18n(): void {
  ...
  if (!referenceLang) {
    console.error('❌ No translation files found to use as reference');
    process.exit(1);
  }
  ...
  for (const translation of translations) {
    ...
    if (extraKeys.length > 0) {
      warnings.push(
        `⚠️  ${lang}: Has ${extraKeys.length} extra key(s) not in reference: ${extraKeys.join(
          ', '
        )}`
      );
    ...
    if (emptyValues.length > 0) {
      warnings.push(
        `⚠️  ${lang}: Has ${emptyValues.length} empty value(s): ${emptyValues.join(', ')}`
      );
