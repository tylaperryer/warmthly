 ...
 */
export function generateTOTP(secret, time) {
 ...
 */
export function verifyTOTP(secret, code, time) {
 ...
 */
export function generateTOTPQRCode(secret, account, issuer) {
    const otpauthUrl = `otpauth://totp/${encodeURIComponent(issuer)}:${encodeURIComponent(account)}?secret=${secret}&issuer=${encodeURIComponent(issuer)}&algorithm=${TOTP_ALGORITHM.toUpperCase()}&digits=${TOTP_DIGITS}&period=${TOTP_PERIOD}`;
 ...
 */
export async function storeTOTPSecret(secret) {
    try {
        ...
        const cipher = crypto.createCipheriv('aes-256-gcm', crypto.createHash('sha256').update(encryptionKey).digest().slice(0, 32), iv);
...
export async function getTOTPSecret() {
    try {
        ...
        // Decrypt
        const decipher = crypto.createDecipheriv('aes-256-gcm', crypto.createHash('sha256').update(encryptionKey).digest().slice(0, 32), Buffer.from(encryptedData.iv, 'hex'));
