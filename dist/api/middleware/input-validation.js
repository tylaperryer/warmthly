...
export function validateString(value, options = {}) {
    const { minLength = 0, maxLength = 10000, pattern, required = false, trim = true, allowEmpty = false, } = options;
 ...
 * @returns Validation result
 */
export function validateCurrency(value, allowedCurrencies = ['USD', 'EUR', 'GBP', 'ZAR', 'CAD', 'AUD']) {
    if (value === null || value === undefined || value === '') {
 ...
 */
export function validateInputWithAttackDetection(value, options = {}) {
        ...
        if (options.logAttack !== false) {
            ...
            import('../security/security-monitor.js').then(({ SecurityLogger }) => {
                if (attack.attackType === 'xss') {
                    ...
                    import('../security/security-monitor.js').then(({ logSecurityEvent, SecurityEventSeverity }) => {
                        logSecurityEvent({
