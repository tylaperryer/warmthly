...
(function (AnomalyType) {
    AnomalyType["TIME_BASED"] = "time_based";
    AnomalyType["LOCATION_BASED"] = "location_based";
    AnomalyType["FREQUENCY_BASED"] = "frequency_based";
    AnomalyType["PATTERN_BASED"] = "pattern_based";
 ...
 */
async function initializeGeoIP(config) {
    if (!config.maxmindDbPath) {
        logger.warn('[anomaly-detection] GeoIP database path not configured, location-based detection disabled');
        return;
    }
    try {
 ...
 */
async function detectTimeBasedAnomaly(event, config) {
 ...
 */
async function detectLocationBasedAnomaly(event, config) {
    ...
    const previousLocation = JSON.parse(storedLocation);
 ...
 */
async function detectFrequencyBasedAnomaly(event, config) {
    const client = await getRedisClient();
    const now = Date.now();
    ...
    if (eventCount > config.maxEventsPerWindow) {
        ...
        return {
            ...
            recommendation: `Unusual frequency of ${event.type} events: ${eventCount} in ${config.windowMs / 1000}s. Verify if this is legitimate.`,
        };
    }
    return null;
}
 ...
 */
export async function detectAnomalies(event, config = DEFAULT_CONFIG) {
    if (!config.enabled) {
        return [];
    }
    const anomalies = [];
    ...
    for (const anomaly of anomalies) {
        ...
        logger.warn(`[anomaly-detection] Anomaly detected: ${anomaly.type} (score: ${anomaly.score})`, anomaly.details);
    }
    return anomalies;
}
/**
 ...
 */
export async function initializeAnomalyDetection(config = DEFAULT_CONFIG) {
    if (!config.enabled) {
        return;
    }
    if (config.locationBased) {
