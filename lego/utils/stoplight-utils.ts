...
export function initStoplight(stoplightId = 'stoplight', menuId = 'dropdown-menu'): void {
  ...
  if (!stoplight || !dropdownMenu) {
    if (import.meta.env.DEV) {
      console.warn(`Stoplight initialization failed: elements not found (stoplight: ${stoplightId}, menu: ${menuId})`);
    }
    return;
  }

  // Ensure elements are correct types
  if (!(stoplight instanceof HTMLElement) || !(dropdownMenu instanceof HTMLElement)) {
    if (import.meta.env.DEV) {
  ...
  function toggleMenu(): void {
    ...
    if (isOpen) {
      ...
      (stoplight as HTMLElement).focus();
      currentFocusIndex = -1;
    }
  }
  ...
  dropdownMenu.addEventListener('focusout', (event) => {
    ...
    setTimeout(() => {
      if (isOpen && !dropdownMenu.contains(document.activeElement) && !stoplight.contains(document.activeElement)) {
        closeMenu();
      }
    }, 0);
  });
