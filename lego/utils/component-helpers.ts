...
export function withLoading<T extends BaseComponent>(
  ...
  return class extends (ComponentClass as any) {
    ...
    protected setLoading(loading: boolean): void {
      ...
      if (
        'setBoolAttr' in this &&
        typeof (this as unknown as { setBoolAttr: (name: string, value: boolean) => void })
          .setBoolAttr === 'function'
      ) {
        (this as unknown as { setBoolAttr: (name: string, value: boolean) => void }).setBoolAttr(
          'loading',
          loading
        );
      }
      if (
        'dispatch' in this &&
        typeof (this as unknown as { dispatch: (event: string, detail: unknown) => void })
          .dispatch === 'function'
      ) {
        (this as unknown as { dispatch: (event: string, detail: unknown) => void }).dispatch(
          'component:loading',
          { loading }
        );
      }
