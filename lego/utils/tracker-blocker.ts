...
function getBlocklistApiUrl(): string {
  if (typeof window !== 'undefined' && (window as unknown as { API_BASE_URL?: string }).API_BASE_URL) {
...
async function resolveCNAME(hostname: string): Promise<string | null> {
  ...
  try {
    ...
    const dohUrl = `https://cloudflare-dns.com/dns-query?name=${encodeURIComponent(hostname)}&type=CNAME`;
...
function isSuspiciousSubdomain(hostname: string): boolean {
  ...
  if (hostname.endsWith(`.${rootDomain}`) && !ALLOWED_DOMAINS.some((d) => matchesDomain(hostname, d))) {
    // Suspicious subdomain patterns
    return /^(analytics|track|pixel|beacon|telemetry|metrics|stats|collect|data|api)\./i.test(hostname);
  }
...
function shouldBlock(url: string | null | undefined): boolean {
  ...
  if (!isBlocked && isSuspiciousSubdomain(hostname)) {
    ...
    shouldBlockAsync(url, true).then((blocked) => {
...
(function initTrackerBlocker(): void {
  ...
  if (typeof WebSocket !== 'undefined') {
    ...
    Object.defineProperty(WebSocketProxy, 'CONNECTING', { value: OriginalWebSocket.CONNECTING, writable: false });
    Object.defineProperty(WebSocketProxy, 'OPEN', { value: OriginalWebSocket.OPEN, writable: false });
    Object.defineProperty(WebSocketProxy, 'CLOSING', { value: OriginalWebSocket.CLOSING, writable: false });
    Object.defineProperty(WebSocketProxy, 'CLOSED', { value: OriginalWebSocket.CLOSED, writable: false });
  ...
  if ('getBattery' in navigator) {
    ...
    (navigator as unknown as { getBattery: () => Promise<BatteryManager> }).getBattery = function (): Promise<BatteryManager> {
      if (import.meta.env.DEV) {
  ...
  if (typeof WebAssembly !== 'undefined') {
    ...
    WebAssembly.instantiate = function (
      ...
      return originalInstantiate.call(this, bytesOrModule, importObject) as unknown as Promise<WebAssembly.WebAssemblyInstantiatedSource>;
          ...
          for (const node of mutation.addedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (element.tagName === 'SCRIPT') {
                ...
                if (shouldBlock(src) || containsTrackingCode(content)) {
                  if (import.meta.env.DEV) {
                    console.warn('[Tracker Blocker] Blocked script in Shadow DOM:', src || 'inline');
                  }
                  element.remove();
                }
              }
  ...
  window.postMessage = function (
    ...
    // If targetOriginOrOptions is a string, check if it's a tracking domain
    if (typeof targetOriginOrOptions === 'string') {
      if (shouldBlock(targetOriginOrOptions) || targetOriginOrOptions === '*') {
        // Block wildcard postMessage (security risk) and tracking domains
        if (import.meta.env.DEV) {
          console.warn('[Tracker Blocker] Blocked postMessage to:', targetOriginOrOptions);
        }
        return;
      }
      // Call with string overload: (message, targetOrigin, transfer?)
      if (transfer !== undefined) {
        (originalPostMessage as (message: unknown, targetOrigin: string, transfer?: Transferable[]) => void).call(this, message, targetOriginOrOptions, transfer);
      } else {
        (originalPostMessage as (message: unknown, targetOrigin: string) => void).call(this, message, targetOriginOrOptions);
      }
      return;
    }

    // If targetOriginOrOptions is an object with origin property
    if (typeof targetOriginOrOptions === 'object' && targetOriginOrOptions !== null) {
      const origin = targetOriginOrOptions.targetOrigin;
      if (origin && (shouldBlock(origin) || origin === '*')) {
        if (import.meta.env.DEV) {
          console.warn('[Tracker Blocker] Blocked postMessage to:', origin);
        }
        return;
      }
      // Call with options overload: (message, options?)
      (originalPostMessage as (message: unknown, options?: WindowPostMessageOptions) => void).call(this, message, targetOriginOrOptions);
      return;
    }
  ...
  (function blockCloudflareInsightsEarly(): void {
    ...
    if (typeof document.querySelector === 'function') {
      document
        .querySelectorAll('script[src*="cloudflareinsights"], script[src*="static.cloudflareinsights"]')
        .forEach((script) => {
          script.remove();
          if (import.meta.env.DEV) {
