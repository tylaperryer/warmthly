...
export const GLOSSARY_DICTIONARY: Record<string, GlossaryTerm> = {
  ...
  'rehumanize': {
    term: 'rehumanize',
    definition: 'To restore human qualities, dignity, and empathy to systems and processes that have become too focused on numbers and efficiency.',
    simplified: 'To make systems care about people again.',
    category: 'Core Concept'
  },
  'radical transparency': {
    term: 'radical transparency',
    definition: 'Complete openness about all operations, decisions, and financial transactions, allowing full public scrutiny.',
    simplified: 'Being completely open about everything we do.',
    category: 'Core Concept'
  },
  'dissolution vote': {
    term: 'dissolution vote',
    definition: 'A community vote to decide whether Warmthly should continue operating. If the vote count reaches 100,000, it triggers a formal process to dissolve the organization.',
    simplified: 'A vote to decide if Warmthly should continue.',
    category: 'Process'
  },
  'transparency': {
    term: 'transparency',
    definition: 'The practice of being open and honest about operations, decisions, and financial matters.',
  ...
  'humility': {
    term: 'humility',
    definition: 'The quality of being modest and respectful, not thinking you are better than others.',
  ...
  'pilot': {
    term: 'pilot',
    definition: 'A small-scale test or trial of a new program or approach before full implementation.',
  ...
  'successor charity': {
    ...
    definition: 'A charity organization that receives assets and continues the mission if the original organization dissolves.',
    simplified: 'Another charity that would continue our work if we stop.',
    category: 'Legal'
  },
  'kill switch': {
    term: 'kill switch',
    definition: 'A mechanism that allows the community to shut down the organization if it fails to uphold its values.',
...
export function initGlossary(): void {
  ...
  contentElements.forEach((element) => {
    ...
    textNodes.forEach((textNode) => {
      ...
      if (!parent || parent.hasAttribute('data-glossary-term') || parent.closest('[data-glossary-term]')) {
        return;
      }
...
function showGlossaryTooltip(element: HTMLElement, term: GlossaryTerm): void {
  ...
  tooltip.innerHTML = `
    ...
    <h3>${term.term}</h3>
    <p>${term.definition}</p>
    ${term.simplified ? `<p class="glossary-simplified"><strong>Simple:</strong> ${term.simplified}</p>` : ''}
  `;
