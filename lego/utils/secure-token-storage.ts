...
async function generateEncryptionKey(purpose: 'access' | 'refresh'): Promise<CryptoKey> {
  ...
  password.fill(0);
  passwordString.split('').forEach((_, i) => {
...
export async function storeAccessToken(token: string): Promise<void> {
  try {
    ...
    throw new Error(`Token storage failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function storeRefreshToken(token: string): Promise<void> {
  try {
    ...
    throw new Error(`Refresh token storage failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
