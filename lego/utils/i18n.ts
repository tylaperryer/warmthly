...
class I18nCache {
  ...
  private async enforceSizeLimits(): Promise<void> {
    ...
    if (totalSize > MAX_CACHE_SIZE || entries.length > MAX_CACHED_LANGUAGES) {
      ...
      for (const entry of entries) {
        if (totalSize <= MAX_CACHE_SIZE && entries.length - toRemove.length <= MAX_CACHED_LANGUAGES) {
          break;
        }
  ...
  async clear(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.db!.transaction(['translations', this.metadataStoreName], 'readwrite');
...
class I18n {
   ...
   */
  private async fetchTranslations(language: string, forceRefresh: boolean = false): Promise<Translations> {
  ...
  async getAvailableLanguages(): Promise<string[]> {
    try {
      ...
      if (import.meta.env.DEV) {
        console.warn('Failed to fetch available languages:', error);
      }
    }
  ...
  async preloadCommonLanguages(): Promise<void> {
    ...
    const commonLanguages = [
      'es', 'fr', 'de', 'it', 'pt', 'ru', 'ja', 'zh', 'ar', 'hi',
      'bn', 'ta', 'te', 'th', 'vi', 'id', 'tr', 'pl', 'nl', 'sv',
    ];
