 ...
 * </warmthly-stoplight>
 */
import { getNavigation } from '/lego/config/warmthly-config.js';
import { getTextSpacingPreferences, toggleTextSpacing } from '/lego/utils/text-spacing.js';
/**
 * Stoplight dot colors
 */
const STOPLIGHT_COLORS = ['red', 'yellow', 'green'];
/**
 * Warmthly Stoplight Web Component
 */
        ...
        try {
            ...
            if (app && !customItems) {
                ...
                spacingToggle.addEventListener('click', e => {
                    ...
                    spacingToggle.setAttribute('aria-checked', !currentPrefs.enabled ? 'true' : 'false');
                ...
                import('/lego/utils/sanitize.js').then(({ sanitizeHtml }) => {
            ...
            void (async () => {
                try {
                    const { initStoplight } = await import('/lego/utils/stoplight-utils.js');
