...
const DEFAULT_DESCRIPTION = 'Warmthly - Rehumanize our world - making empathy a measurable part of our systems';
...
function generateHowToSchema(): object {
  return {
    ...
    description: 'Step-by-step guide to making a donation to support Warmthly\'s mission to make empathy a measurable part of our systems.',
    step: [
      {
        '@type': 'HowToStep',
        position: 1,
...
function generateVideoObjectSchema(
  ...
  return {
    ...
    embedUrl: videoUrl ? videoUrl.replace('/watch?v=', '/embed/') : 'https://www.youtube-nocookie.com/embed/kVausES-mjk',
...
class WarmthlyHead extends HTMLElement {
  ...
  async connectedCallback(): Promise<void> {
    ...
    try {
      ...
      const image = this.getAttribute('image') || getDynamicOGImage(app, window.location.pathname, title, description);
...
`;
      ...
      const cssPreloadLinks = appCssFiles
        .map((css) => `  <link rel="preload" href="${stylesPath}/${css}" as="style" onload="this.onload=null;this.rel='stylesheet'">`)
        .join('\n');
      ...
      const robotsMeta = robots ? `  <meta name="robots" content="${this.escapeHtml(robots)}">\n` : '';
      
      // Generate last-modified meta tag (SEO Enhancement - Last Updated Dates)
      const lastModifiedMeta = lastModified ? `  <meta name="last-modified" content="${this.escapeHtml(lastModified)}">\n` : '';
      ...
      if (app === 'main' && (window.location.pathname === '/' || window.location.pathname === '/index.html')) {
      ...
      if (app === 'main' && window.location.pathname.includes('video')) {
        structuredDataArray.push(generateVideoObjectSchema(
          title,
          description,
          'https://www.youtube.com/watch?v=kVausES-mjk',
          image
        ));
      } else if (app === 'main' && (window.location.pathname === '/' || window.location.pathname === '/index.html')) {
...
${structuredDataArray.map((data) => `  <script type="application/ld+json">
