...
import { validateEmail, validateString, validateUrl, type ValidationResult } from '@api/middleware/input-validation.js';
...
class WarmthlyInput extends BaseComponent {
  ...
  protected onAttributeChange(name: string, oldValue: string | null, newValue: string | null): void {
    ...
    switch (name) {
      ...
      case 'pattern':
        ...
        break;
      case 'autocomplete':
        this.inputElement.autocomplete = newValue || 'off';
        break;
      case 'label':
  ...
  private createInput(): void {
    ...
    this.inputElement.disabled = this.hasAttr('disabled');
    this.inputElement.readOnly = this.hasAttr('readonly');
    this.inputElement.autocomplete = this.getAttr('autocomplete') || 'off';
