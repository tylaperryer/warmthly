...
import { validateString, type ValidationResult } from '@api/middleware/input-validation.js';
...
class WarmthlyTextarea extends BaseComponent {
  ...
  private createTextarea(): void {
    ...
    if (labelText) {
      ...
      const textareaId = this.getAttr('id') || `textarea-${Math.random().toString(36).substr(2, 9)}`;
    ...
    this.textareaElement.id = this.getAttr('id') || `textarea-${Math.random().toString(36).substr(2, 9)}`;
