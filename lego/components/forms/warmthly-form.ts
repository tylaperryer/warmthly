...
import { validateEmail, validateString, type ValidationResult } from '@api/middleware/input-validation.js';
...
class WarmthlyForm extends BaseComponent {
  ...
  private setupForm(): void {
    ...
    this.submitButton = this.formElement.querySelector('button[type="submit"], warmthly-button[type="submit"]') as HTMLButtonElement;
  }
  ...
  private validateForm(): { valid: boolean; errors?: Record<string, string> } {
    ...
    const fields = Array.from(this.formElement.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>(
  ...
  private showErrors(errors: Record<string, string>): void {
    ...
    for (const [fieldName, errorMessage] of Object.entries(errors)) {
      const field = this.formElement?.querySelector<HTMLElement>(`[name="${fieldName}"], warmthly-input[name="${fieldName}"], warmthly-textarea[name="${fieldName}"], warmthly-select[name="${fieldName}"]`);
      if (field) {
  ...
  private setFormDisabled(disabled: boolean): void {
    ...
    const fields = this.formElement.querySelectorAll<HTMLElement>('input, textarea, select, button, warmthly-input, warmthly-textarea, warmthly-select, warmthly-button');
    fields.forEach((field) => {
