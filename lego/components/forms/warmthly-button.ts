...
class WarmthlyButton extends BaseComponent {
  ...
  protected onAttributeChange(name: string, oldValue: string | null, newValue: string | null): void {
    ...
    switch (name) {
      case 'type':
        this.buttonElement.type = (newValue || 'button').toLowerCase() as 'button' | 'submit' | 'reset';
        break;
      case 'variant':
  ...
  private createButton(): void {
    ...
    this.buttonElement.type = (this.getAttr('type') || 'button').toLowerCase() as 'button' | 'submit' | 'reset';
    this.buttonElement.disabled = this.hasAttr('disabled');
  ...
  private updateVariant(variant: string | null): void {
    ...
    this.buttonElement.classList.remove('variant-primary', 'variant-secondary', 'variant-danger', 'variant-ghost');
