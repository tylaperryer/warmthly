...
class WarmthlyFieldGroup extends BaseComponent {
  ...
  private createFieldGroup(): void {
    ...
    this.fieldElement = this.query<HTMLElement>('warmthly-input, warmthly-textarea, warmthly-select, input, textarea, select');
    ...
    if (!this.fieldElement) {
      ...
      this.fieldElement = wrapper.querySelector<HTMLElement>('warmthly-input, warmthly-textarea, warmthly-select, input, textarea, select') || wrapper;
    }
    ...
    if (helpText) {
      ...
      if (helpLink) {
        ...
        helpIconLink.style.cssText = 'color: var(--warmthly-orange); text-decoration: none; font-weight: bold; margin-left: 0.25rem;';
      ...
      if (this.fieldElement) {
        ...
        this.fieldElement.setAttribute('aria-describedby', `${describedBy} ${this.helpElement.id}`.trim());
      }
  ...
  private updateError(text: string | null): void {
    ...
    if (text) {
      ...
      // Link error to field
      if (this.fieldElement) {
        const fieldId = this.errorElement.id || `error-${this.getFieldId() || Math.random().toString(36).substr(2, 9)}`;
