...
class WarmthlyLanguageSwitcher extends HTMLElement {
  ...
  private render(): void {
    const currentLang = this.languages.find(l => l.code === this.currentLanguage) || 
                       { code: this.currentLanguage, name: this.currentLanguage.toUpperCase(), nativeName: this.currentLanguage.toUpperCase(), locale: `${this.currentLanguage}_${this.currentLanguage.toUpperCase()}`, rtl: false };
        ...
        ${this.isOpen ? `
          <div class="language-switcher__dropdown" role="listbox" id="language-switcher-dropdown">
            ...
            <div class="language-switcher__list" role="list">
              ${this.filteredLanguages.length > 0 ? this.filteredLanguages.map((lang, index) => `
                <button
                ...
                >
                  ...
                  ${lang.rtl ? '<span class="language-switcher__rtl-indicator" aria-label="Right-to-left language">RTL</span>' : ''}
                </button>
              `).join('') : `
            ...
            <div class="language-switcher__footer">
              <span class="language-switcher__count">${this.filteredLanguages.length} of ${this.languages.length} languages</span>
            </div>
          </div>
        ` : ''}
      </div>
  ...
  private attachEventListeners(): void {
    ...
    this.addEventListener('keydown', (e) => {
      ...
      const options = Array.from(this.querySelectorAll('.language-switcher__option')) as HTMLElement[];
