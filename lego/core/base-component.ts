...
export abstract class BaseComponent extends HTMLElement implements ComponentLifecycle {
  ...
  private attributeChangeQueue: Array<{ name: string; oldValue: string | null; newValue: string | null }> = [];
   ...
   */
  private processAttributeChange(name: string, oldValue: string | null, newValue: string | null): void {
    this.safeExecute(() => {
   ...
   */
  protected dispatch<T = unknown>(eventName: string, detail?: T, bubbles = true, cancelable = true): boolean {
